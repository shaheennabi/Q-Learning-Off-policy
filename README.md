...
- \( \max_{a'} Q(s', a') \) assumes greedy action selection in the next state  

Because the update uses the **greedy target policy**, while actions may be generated by  
an exploratory behavior policy (e.g., ε-greedy), **Q-learning is off-policy**.
